 ...
 */
package studio.lively.scl.gradle.ci;
...
public abstract class CheckUpdate extends DefaultTask {
    ...
    public void run() throws Exception {
        ...
        try (PrintWriter writer = githubEnv.isBlank()
            ...
            addEnv.accept("SCL_COMMIT_SHA", buildMetadata.revision());
            addEnv.accept("SCL_VERSION", buildMetadata.version());
            addEnv.accept("SCL_TAG_NAME", "v" + buildMetadata.version());
            addEnv.accept("SCL_CI_DOWNLOAD_BASE_URI", buildMetadata.downloadBaseUri);
        }
    }
    ...
    private BuildMetadata fetchBuildInfo(Helper helper, String jobUri) throws IOException, InterruptedException {
        ...
        Pattern fileNamePattern = Pattern.compile("SCL-(?<version>\\d+(?:\\.\\d+)+)\\.jar");
