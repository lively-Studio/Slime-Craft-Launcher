 ...
 */
package studio.lively.scl.util.gson;
...
/// Stores a raw JSON schema string and, when possible, its parsed SCL schema identifier.
///
/// The JSON representation is always a string. SCL-owned schemas use the following
/// fixed URL form, but other strings can still be represented as unparseable schemas:
///
/// `https://raw.githubusercontent.com/lively-Studio/Slime-Craft-Launcher/main/docs/schemas/<id>/<version>`
///
...
///
/// Parsed SCL schemas use the following compatibility policy:
///
/// - When the schema string is not parseable as an SCL schema URL, the file must be rejected.
...
/// @param parsed the parsed SCL schema identifier, or `null` when the string is not parseable
/// @author Glavo
@JsonSerializable
...
public record JsonSchema(String value, @Nullable Parsed parsed) {
    ...
    /// The SCL schema URL prefix.
    private static final String URL_PREFIX = "https://raw.githubusercontent.com/lively-Studio/Slime-Craft-Launcher/main/docs/schemas/";
    ...
    /// Creates a parsed SCL schema from an ID and version.
    ///
    ...
    /// Creates a schema from a parsed SCL schema identifier.
    ...
    /// Parses an SCL schema URL, returning `null` for any other string.
    ...
    /// Returns whether this schema string is parseable as an SCL schema URL.
    public boolean isParsed() {
        return parsed != null;
    }

    /// Returns the parsed SCL schema ID, or `null` when the schema string is not parseable.
    public @Nullable String id() {
        return parsed != null ? parsed.id : null;
    }

    /// Returns the parsed SCL schema version, or `null` when the schema string is not parseable.
    ...
    /// Parsed identifier for an SCL schema URL.
    ///
    /// @param id the stable schema identifier
    /// @param version the schema version
    @NotNullByDefault
