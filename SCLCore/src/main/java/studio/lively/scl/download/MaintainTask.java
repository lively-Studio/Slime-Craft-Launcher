...
import studio.lively.scl.util.SimpleMultimap;
...
import static studio.lively.scl.download.LibraryAnalyzer.LibraryType.*;
...
public class MaintainTask extends Task<Version> {
    ...
    public static Version maintain(GameRepository repository, Version version) {
        ...
        if (!libraries.isEmpty()) {
            // SCL once use log4j-patch to prevent virus. But now, we only modify log4j2.xml.
    ...
    private static Version maintainGameWithCpwModLauncher(GameRepository repository, Version version) {
        ...
        if (libraryAnalyzer.has(OPTIFINE)) {
            Library sclTransformerDiscoveryService = new Library(new Artifact("studio.lively.scl", "transformer-discovery-service", "1.0"));
            Optional<Library> optiFine = version.getLibraries().stream().filter(library -> library.is("optifine", "OptiFine")).findAny();
            boolean libraryExisting = version.getLibraries().stream().anyMatch(library -> library.is("studio.lively.scl", "transformer-discovery-service"));
            optiFine.ifPresent(library -> {
                builder.addJvmArgument("-Dscl.transformer.candidates=${library_directory}/" + library.getPath());
                if (!libraryExisting) builder.addLibrary(sclTransformerDiscoveryService);
                Path libraryPath = repository.getLibraryFile(version, sclTransformerDiscoveryService);
                try (InputStream input = MaintainTask.class.getResourceAsStream("/assets/game/SCLTransformerDiscoveryService-1.0.jar")) {
                    Files.createDirectories(libraryPath.getParent());
                    Files.copy(Objects.requireNonNull(input, "Bundled SCLTransformerDiscoveryService is missing."), libraryPath, StandardCopyOption.REPLACE_EXISTING);
                } catch (IOException | NullPointerException e) {
                    LOG.warning("Unable to unpack SCLTransformerDiscoveryService", e);
                }
            });
        }
