...
SCL provides localization support for multiple languages.

This document describes SCL's support status for these languages and provides a guide for contributors who want to help with SCL localization.

## Supported languages

Currently, SCL supports the following languages:
...
SCL uses language tags that conform to the IETF BCP 47 standard.
...
SCL will derive a search list from the current language tag and search for resources in order.
...
SCL requires all pull requests that update documentation and localization resources to also update the resources
...
SCL welcomes anyone to participate in translation and contribution. 
...
We recommend that contributors submit a [feature request](https://github.com/lively-Studio/Slime-Craft-Launcher/issues/new?template=feature.yml) before providing a new language translation,
...
If you want to add support for a new language to SCL, please start by translating [`I18N.properties`](../SCL/src/main/resources/assets/lang/I18N.properties).
The vast majority of SCL's texts are in this file, and translating it will translate the entire interface.
...
After determining the language tag, please create a file named `I18N_<localization file suffix>.properties` (e.g., `I18N_en.properties`) next to the [`I18N.properties` file](../SCL/src/main/resources/assets/lang).
...
After translating part of the file, you can [build SCL yourself](./Contributing.md#build-SCL), 
and your translations will be included in the compiled SCL.
If your computer's default environment is not the target language,
you can set the environment variable `SCL_LANGUAGE` to the language tag you just found from the table,
and SCL will automatically switch to that language.

At this point, you can push the file to GitHub and submit a PR to SCL.
The maintainers of SCL will complete the remaining steps for you.
...
In addition to localized files, SCL also supports localizing certain text fields in JSON. Localized text in JSON uses the following format:
...
For missing resources in a certain language, SCL supports a resource fallback mechanism,
...
1. Normalize Language Codes
   ...
   SCL will try to map it to a registered tag.

   For example, SCL will replace the language code `eng` with `en`.
...
2. Map Macrolanguages to Individual Languages
   ...
   and the macrolanguage usually refers to a specific individual language, SCL will replace it with that individual language.

   For example, `zh` (Chinese) usually actually refers to `cmn` (Mandarin), so we replace the language code `zh` with `cmn`.

3. Derive Script

   If the current language tag does not specify a script, SCL will try to derive the script according to the following rules in order:
...
After refining the language code, SCL will derive a list of language tags based on this language tag.

For example, for the language tag `en-US`, SCL will refine it to `en-Latn-US` and derive the following search list:
...
For the language tag `zh-CN`, SCL will refine it to `cmn-Hans-CN` and derive the following search list:
...
SCL will merge resources according to the priority of this list; for resources that are difficult to merge (such as font files), 
SCL will load the highest-priority resource found in this list.

If the current language uses a three-letter ISO 639 code, but there is also a corresponding two-letter code, 
SCL will map it to the two-letter code before searching for resources.

For example, if the current environment's language tag is `eng-US`, 
SCL will map it to `en-US` and then search for localization resources according to the above rules.

### Additional Rules for Chinese

SCL always adds `zh-CN` to the search list for all Chinese environments, 
...
To simplify documentation maintenance, SCL uses a macro mechanism to automatically maintain parts of the documentation content.
...
For more about macros, see [MacroProcessor.java](../buildSrc/src/main/java/studio/lively/scl/gradle/docs/MacroProcessor.java).
<!-- #END BLOCK -->
