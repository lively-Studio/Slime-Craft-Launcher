# Slime Craft Launcher Release Schedule

<!-- #BEGIN LANGUAGE_SWITCHER -->
**English** | 中文 ([简体](ReleaseSchedule_zh.md), [繁體](ReleaseSchedule_zh_Hant.md))
<!-- #END LANGUAGE_SWITCHER -->

This document describes the SCL version release schedule starting from October 2025.

## Versioning Rules

### Development Version Format (Current)

Since 2026, SCL uses prefixed version strings to distinguish version types:

| Type   | Prefix | Format                                     | Example               |
|--------|--------|--------------------------------------------|-----------------------|
| Dev    | `DEV`  | `DEV{year}.{major}.{minor}-SNAPSHOT-{build}` | `DEV2026.1.0-SNAPSHOT-2` |
| Stable | `V`    | `V{major}.{minor}.{patch}`                   | `V3.17.1`             |
...
| Segment     | Meaning              | Example |
|-------------|----------------------|---------|
| `V`         | Stable release (普通版)| `V`     |
| `{major}`   | Major version        | `3`     |
| `{minor}`   | Minor version        | `17`    |
| `{patch}`   | Patch version        | `1`     |

### Legacy Version Branches

SCL has multiple **version branches**, each named in the format `<major>.<minor>` (e.g., `3.7`).

The major version is incremented only when there are significant architectural changes in SCL,
while the minor version is incremented regularly according to the release schedule.

### Version Types

SCL has two types of versions: **Stable** and **Beta**.

#### Stable Version

The SCL stable version is suitable for users who prioritize software stability.
...
The SCL beta version is suitable for users who want to experience new features first.
...
SCL has two main release channels: **Stable Channel** and **Beta Channel**.
They are used to release the SCL stable and beta versions, respectively.

To test SCL versions before official release, we will push updates to some users in advance.
Users can enable the "Preview SCL releases early" option on the "Settings > General" page to receive preview updates from the corresponding channel.
...
---
gantt
    title SCL Version Lifecycle (Example)
    section 3.11
