...
import studio.lively.scl.util.SelfDependencyPatcher;
import studio.lively.scl.util.SwingUtils;
...
public final class EntryPoint {
    ...
    public static void main(String[] args) {
        ...
        System.getProperties().putIfAbsent("http.agent", "SCL/" + Metadata.VERSION);

        createSCLDirectories();
        LOG.start(Metadata.SCL_LOCAL_HOME.resolve("logs"));
    ...
    private static void createSCLDirectories() {
        if (!Files.isDirectory(Metadata.SCL_LOCAL_HOME)) {
            try {
                Files.createDirectories(Metadata.SCL_LOCAL_HOME);
                if (OperatingSystem.CURRENT_OS == OperatingSystem.WINDOWS) {
                    try {
                        Files.setAttribute(Metadata.SCL_LOCAL_HOME, "dos:hidden", true);
                    } catch (IOException e) {
                        LOG.warning("Failed to set hidden attribute of " + Metadata.SCL_LOCAL_HOME, e);
                    }
                }
            } catch (IOException e) {
                // Logger has not been started yet, so print directly to System.err
                System.err.println("Failed to create SCL directory: " + Metadata.SCL_LOCAL_HOME);
                e.printStackTrace(System.err);
                showErrorAndExit(i18n("fatal.create_hmcl_current_directory_failure", Metadata.SCL_LOCAL_HOME));
            }
        }

        if (!Files.isDirectory(Metadata.SCL_USER_HOME)) {
            try {
                Files.createDirectories(Metadata.SCL_USER_HOME);
            } catch (IOException e) {
                LOG.warning("Failed to create SCL user home " + Metadata.SCL_USER_HOME, e);
            }
        }
    }
    ...
    private static void checkWine() {
        if (OperatingSystem.isRunningUnderWine()) {
            LOG.warning("SCL is running under Wine or its distributions!");
