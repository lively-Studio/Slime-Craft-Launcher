...
import studio.lively.scl.game.JavaVersionConstraint;
...
import studio.lively.scl.util.platform.UnsupportedPlatformException;
import studio.lively.scl.util.platform.windows.WinReg;
...
public final class JavaManager {
    ...
    public static final SCLJavaRepository REPOSITORY = new SCLJavaRepository(Metadata.SCL_USER_HOME.resolve("java"));
    public static final SCLJavaRepository LOCAL_REPOSITORY = new SCLJavaRepository(Metadata.SCL_LOCAL_HOME.resolve("java"));
    ...
    private static Map<Path, JavaRuntime> searchPotentialJavaExecutables(boolean useCache) {
        Searcher searcher = new Searcher(Metadata.SCL_USER_HOME.resolve("javaCache.json"));
        if (useCache)
            searcher.loadCache();
        ...
        if (System.getenv("PATH") != null) {
            ...
            for (String path : paths) {
                // https://github.com/SCL-dev/SCL/issues/4079
        ...
        if (System.getenv("SCL_JRES") != null) {
            String[] paths = System.getenv("SCL_JRES").split(File.pathSeparator);
            for (String path : paths) {
                try {
        ...
        if (!JavaInfoUtils.canSpawnProcess()) {
            LOG.warning("Current JVM cannot spawn child processes (posix_spawn broken). "
                    + "Java runtimes are still listed but game launching may fail. "
                    + "A retry mechanism will be used when launching Minecraft.");
        }
