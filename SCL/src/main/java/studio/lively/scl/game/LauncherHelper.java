...
import studio.lively.scl.Launcher;
import studio.lively.scl.auth.*;
import studio.lively.scl.auth.authlibinjector.AuthlibInjectorDownloadException;
import studio.lively.scl.auth.offline.OfflineAccount;
...
import studio.lively.scl.download.game.*;
import studio.lively.scl.java.JavaInfoUtils;
import studio.lively.scl.java.JavaManager;
import studio.lively.scl.java.JavaRuntime;
import studio.lively.scl.launch.*;
import studio.lively.scl.modpack.ModpackCompletionException;
...
import studio.lively.scl.setting.JavaVersionType;
import studio.lively.scl.setting.LauncherVisibility;
import studio.lively.scl.task.*;
...
import studio.lively.scl.util.*;
import studio.lively.scl.util.i18n.I18n;
import studio.lively.scl.util.io.FileUtils;
import studio.lively.scl.util.io.ResponseCodeException;
...
import static studio.lively.scl.util.DataSizeUnit.MEGABYTES;
import static studio.lively.scl.util.Lang.resolveException;
import static studio.lively.scl.util.i18n.I18n.i18n;
import static studio.lively.scl.util.logging.Logger.LOG;
import static studio.lively.scl.util.logging.Logger.TIME_FORMATTER;
import static studio.lively.scl.util.platform.Platform.SYSTEM_PLATFORM;
...
public final class LauncherHelper {
    ...
    public LauncherHelper(SCLGameRepository repository, Account account, String selectedVersion) {
    ...
    private void launch0() {
        // https://github.com/SCL-dev/SCL/pull/4121
        ...
        TaskExecutor executor = checkGameState(repository, setting, version.get())
                ...
                .thenComposeAsync(authInfo -> Task.supplyAsync(() -> {
                    ...
                    return new SCLGameLauncher(
                            ...
                            launcherVisibility == LauncherVisibility.CLOSE
                                    ...
                                    : new SCLProcessListener(repository, version.get(), authInfo, launchOptions, launchingLatch, gameVersion.isPresent())
                    );
                ...
                .withStagesHints(
                        ...
                        new Task.StagesHint("launch.state.dependencies", List.of("scl.install.assets", "scl.install.libraries", "scl.modpack.download")),
    ...
    private static Task<JavaRuntime> checkGameState(SCLGameRepository repository, GameSettings.Effective setting, Version version) {
        ...
        if (setting.getInheritable(GameSettings::notCheckJVMProperty)) {
            ...
            task = getJavaTask.thenComposeAsync(java -> {
                ...
                // Warn the user if the current JVM has known posix_spawn issues.
                if (!JavaInfoUtils.canSpawnProcess()) {
                    LOG.warning("Current JVM has posix_spawn issues. "
                            + "Game launch may fail; SCL will automatically retry if so.");
                }

                // Pre-launch check: verify the game's Java can actually run
                if (java != null && java.getBinary() != null) {
                    try {
                        if (!JavaInfoUtils.canSpawnProcess()) {
                            LOG.warning("Skipping pre-launch Java health check (current JVM cannot spawn)");
                        } else {
                            ProcessBuilder testPb = new ProcessBuilder(
                                    java.getBinary().toString(), "-version");
                            Process testProc = testPb.start();
                            int exitCode = testProc.waitFor();
                            if (exitCode != 0) {
                                LOG.warning("Game Java health check failed: " + java.getBinary()
                                        + " exited with code " + exitCode);
                            }
                        }
                    } catch (Exception e) {
                        FXUtils.runInFX(() -> Controllers.dialog(
                                i18n("launch.failed.executing_jvm", java.getBinary()),
                                i18n("message.error"),
                                MessageType.ERROR,
                                breakAction
                        ));
    ...
    private static CompletableFuture<JavaRuntime> downloadJava(GameJavaVersion javaVersion, SCLGameRepository repository) {
        CompletableFuture<JavaRuntime> future = new CompletableFuture<>();
     ...
     * Guarantee that one [JavaProcess], one [SCLProcessListener].
     * Because every time we launched a game, we generates a new [SCLProcessListener]
     */
    private final class SCLProcessListener implements ProcessListener {
        ...
        public SCLProcessListener(SCLGameRepository repository, Version version, AuthInfo authInfo, LaunchOptions launchOptions, CountDownLatch launchingLatch, boolean detectWindow) {
        ...
        public void onExit(int exitCode, ExitType exitType) {
            if (showLogs) {
                logBuffer.add(new Log(String.format("[%s] [SCL ProcessListener] Minecraft exit with code %d(0x%x), type is %s.", TIME_FORMATTER.format(Instant.now()), exitCode, exitCode, exitType), Log4jLevel.INFO));
