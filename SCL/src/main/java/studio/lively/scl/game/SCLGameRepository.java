...
import studio.lively.scl.event.EventManager;
...
import studio.lively.scl.setting.LauncherSettings;
import studio.lively.scl.setting.SettingsManager;
import studio.lively.scl.setting.DefaultIsolationType;
...
import studio.lively.scl.setting.LegacyGameSettingsMigrator;
import studio.lively.scl.setting.GameDirectory;
import studio.lively.scl.setting.ProxyType;
import studio.lively.scl.setting.SettingFileUtils;
import studio.lively.scl.setting.GameSettingsPresetID;
import studio.lively.scl.setting.VersionIconType;
import studio.lively.scl.ui.FXUtils;
import studio.lively.scl.util.FileSaver;
...
import studio.lively.scl.util.platform.SystemInfo;
...
/// SCL game repository implementation backed by a GameDirectory and per-instance game settings.
@NotNullByDefault
public final class SCLGameRepository extends DefaultGameRepository {
    ...
    @NotNullByDefault
    public record InstanceReference(SCLGameRepository repository, @Nullable String instanceId) {
    }

    /// Directory under the version root that stores SCL-managed instance metadata.
    private static final String INSTANCE_METADATA_DIRECTORY = ".scl";
    ...
    public SCLGameRepository(GameDirectory gameDirectory) {
    ...
    public DefaultDependencyManager getDependency(DownloadProvider downloadProvider) {
        return new DefaultDependencyManager(this, downloadProvider, SCLCacheRepository.REPOSITORY);
    }
    ...
    /// Returns the SCL-managed metadata directory under the version root.
    ///
    /// This directory stores instance-scoped files owned by SCL.
    public Path getInstanceMetadataDirectory(String id) {
        return getVersionRoot(id).resolve(INSTANCE_METADATA_DIRECTORY);
    }

    /// Returns the SCL-managed configuration directory under the instance metadata directory.
    public Path getInstanceConfigDirectory(String id) {
        return getInstanceMetadataDirectory(id).resolve(INSTANCE_CONFIG_DIRECTORY);
    }

    /// Returns the SCL-managed state directory under the instance metadata directory.
    ...
    public static long getAutoAllocatedMemory(long available) {
        long usable = available - 512 * 1024 * 1024; // Reserve 512 MiB memory for off-heap memory and SCL itself
        if (usable <= 0) {
            return available;
        }
