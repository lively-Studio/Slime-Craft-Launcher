...
import static studio.lively.scl.ui.FXUtils.stringConverter;
import static studio.lively.scl.util.i18n.I18n.i18n;
import static studio.lively.scl.util.javafx.ExtendedProperties.selectedItemPropertyFor;
...
public class DownloadListPage extends Control implements DecoratorPage, VersionPage.GameInstanceLoadable {
    ...
    public void loadInstance(SCLGameRepository repository, @Nullable String instanceId) {
        this.instanceReference.set(new SCLGameRepository.InstanceReference(repository, instanceId));
    ...
    private void search(String userGameVersion, RemoteAddonRepository.Category category, int pageOffset, String searchFilter, RemoteAddonRepository.SortType sort) {
        ...
        Task.supplyAsync(() -> {
            SCLGameRepository.InstanceReference instanceReference = this.instanceReference.get();
    ...
    protected SCLGameRepository.InstanceReference getInstanceReference() {
        if (versionSelection) {
            return new SCLGameRepository.InstanceReference(instanceReference.get().repository(), selectedVersion.get());
        } else {
