...
import studio.lively.scl.game.LauncherHelper;
...
import studio.lively.scl.ui.account.AccountListPage;
import studio.lively.scl.ui.animation.AnimationUtils;
import studio.lively.scl.ui.animation.ContainerAnimations;
import studio.lively.scl.ui.animation.Motion;
import studio.lively.scl.ui.construct.*;
import studio.lively.scl.ui.construct.MessageDialogPane.MessageType;
import studio.lively.scl.ui.decorator.DecoratorController;
import studio.lively.scl.ui.download.DownloadPage;
import studio.lively.scl.ui.main.LauncherSettingsPage;
import studio.lively.scl.ui.main.RootPage;
import studio.lively.scl.ui.nbt.NBTEditorPage;
import studio.lively.scl.ui.terracotta.TerracottaPage;
import studio.lively.scl.ui.versions.GameListPage;
...
import studio.lively.scl.util.i18n.SupportedLocale;
...
public final class Controllers {
    ...
    public static final int MIN_CONTENT_WIDTH = 442 + 2; // width + border
    ...
    /// Opens a file chooser for NBT files (.nbt, .dat, .dat_old, .mca, .mcr)
    /// and navigates to the NBT editor page with the selected file.
    @FXThread
    public static void openNbtEditor() {
        FileChooser chooser = new FileChooser();
        chooser.setTitle(i18n("nbt.viewer"));
        chooser.getExtensionFilters().add(new FileChooser.ExtensionFilter(
                i18n("nbt.viewer") + " (*.nbt, *.dat, *.dat_old, *.mca, *.mcr)",
                "*.nbt", "*.dat", "*.dat_old", "*.mca", "*.mcr"));
        java.io.File selected = chooser.showOpenDialog(getStage());
        if (selected == null) {
            return;
        }
        Path file = selected.toPath();
        try {
            NBTEditorPage page = new NBTEditorPage(file);
            navigate(page);
        } catch (IOException e) {
            LOG.warning("Failed to open NBT file " + file, e);
            dialog(i18n("nbt.open.failed") + ": " + e.getLocalizedMessage());
        }
    }
    ...
    public static void initialize(Stage stage) {
        ...
        stageSizeChangeListener = o -> {
            ...
            boolean saveState = !currentStage.isIconified()
                    // https://github.com/SCL-dev/SCL/issues/4290
    ...
    public static void onHyperlinkAction(String href) {
        if (href.startsWith("scl://")) {
            switch (href) {
                case "scl://settings/feedback":
                    Controllers.getSettingsPage().showFeedback();
                    Controllers.navigate(Controllers.getSettingsPage());
                    break;
                case "scl://game/launch":
