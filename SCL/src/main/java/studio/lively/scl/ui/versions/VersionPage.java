...
import studio.lively.scl.event.EventPriority;
...
public class VersionPage extends DecoratorAnimatedPage implements DecoratorPage {
    ...
    private final ObjectProperty<SCLGameRepository.InstanceReference> instanceReference = new SimpleObjectProperty<>();
    ...
    private void checkSelectedVersion() {
        runInFX(() -> {
            ...
            SCLGameRepository repository = this.instanceReference.get().repository();
    ...
    public void setVersion(String version, SCLGameRepository repository) {
        this.instanceReference.set(new SCLGameRepository.InstanceReference(repository, version));
    }

    public void loadVersion(String version, SCLGameRepository repository) {
    ...
    private void clearAssets() {
        ...
        SCLGameRepository.InstanceReference currentInstanceReference = instanceReference.get();
    ...
    public SCLGameRepository getRepository() {
        return Optional.ofNullable(instanceReference.get()).map(SCLGameRepository.InstanceReference::repository).orElse(null);
    }

    public String getVersion() {
        return Optional.ofNullable(instanceReference.get()).map(SCLGameRepository.InstanceReference::instanceId).orElse(null);
    }
    ...
    public static class Skin extends DecoratorAnimatedPageSkin<VersionPage> {
        ...
        protected Skin(VersionPage control) {
...

            {
                ...
                sideBar.addNavigationDrawerTab(control.tab, control.versionSettingsTab, i18n("settings.game"), SVG.SETTINGS, SVG.SETTINGS_FILL)
                ...
                AdvancedListBox toolbar = new AdvancedListBox(true /* horizontal */);
                toolbar.addNavigationDrawerItem(i18n("version.update"), SVG.UPDATE, control::updateGame, upgradeItem -> {
    ...
    public interface GameInstanceLoadable {
        ...
        void loadInstance(SCLGameRepository repository, @Nullable String instanceId);
    }
}
