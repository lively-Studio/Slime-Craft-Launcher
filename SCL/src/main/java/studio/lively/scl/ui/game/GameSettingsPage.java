 ...
 */
package studio.lively.scl.ui.game;
...
import studio.lively.scl.java.JavaManager;
import studio.lively.scl.java.JavaRuntime;
import studio.lively.scl.setting.*;
import studio.lively.scl.setting.property.InheritableProperty;
import studio.lively.scl.setting.property.SettingProperty;
import studio.lively.scl.ui.*;
import studio.lively.scl.ui.construct.*;
import studio.lively.scl.ui.decorator.DecoratorPage;
import studio.lively.scl.ui.versions.VersionIconDialog;
import studio.lively.scl.ui.versions.VersionPage;
import studio.lively.scl.util.Holder;
import studio.lively.scl.util.Pair;
import studio.lively.scl.util.ServerAddress;
...
import static studio.lively.scl.util.Pair.pair;
...
public final class GameSettingsPage<S extends GameSettings> extends StackPane
    ...
    private @Nullable SCLGameRepository repository;
    ...
    public GameSettingsPage(Class<S> settingType) {
        ...
        var launcherSettings = new ComponentList();
        {
            ...
            currentSetting.addListener((o, oldSetting, newSetting) -> {
                if (oldSetting != null) {
                    oldSetting.javaTypeProperty().removeListener(weakJavaListener);
                    oldSetting.detectedJavaProperty().removeListener(weakJavaListener);
                    oldSetting.customJavaPathProperty().removeListener(weakJavaListener);
                    oldSetting.customJavaVersionProperty().removeListener(weakJavaListener);
                    if (oldSetting instanceof GameSettings.Instance oldInstance) {
                        oldInstance.parentProperty().removeListener(weakJavaListener);
                    }
                }

                if (newSetting != null) {
                    newSetting.javaTypeProperty().addListener(weakJavaListener);
                    newSetting.detectedJavaProperty().addListener(weakJavaListener);
                    newSetting.customJavaPathProperty().addListener(weakJavaListener);
                    newSetting.customJavaVersionProperty().addListener(weakJavaListener);
                    if (newSetting instanceof GameSettings.Instance newInstance) {
                        newInstance.parentProperty().addListener(weakJavaListener);
                    }
                }
            ...
            windowTypeSublist.setHasSubtitle(true);
            {
                ...
                @Nullable JFXButton windowSizeInheritanceButton = !isPresetSetting ? createInheritanceButton() : null;
                bindWindowSizeComboBox(cboWindowSize, windowSizeInheritanceButton);

                for (GameWindowType type : GameWindowType.values()) {
                    if (type == GameWindowType.WINDOWED) {
                        windowTypeOptions.add(new EditorChoice<>(
                                getWindowTypeDisplayName(GameWindowType.WINDOWED),
                                GameWindowType.WINDOWED,
                                cboWindowSize,
                                windowSizeInheritanceButton));
                    } else {
            ...
            var quickSublist = new ComponentSublist();
            {
                ...
                var multiplayerOption = new EditorChoice<>(
                        i18n("settings.game.quick_play.multiplayer"),
                        QuickPlayType.MULTIPLAYER,
                        multiplayerTextField,
                        multiplayerInheritanceButton);

                var singleplayerTextField = new JFXTextField();
                singleplayerTextField.setValidators(new Validator(str -> {
                    if (StringUtils.isBlank(str))
                        return true;
                    return FileUtils.isNameValid(str);
                }));
                FXUtils.setValidateWhileTextChanged(singleplayerTextField, true);
                @Nullable JFXButton singleplayerInheritanceButton = !isPresetSetting ? createInheritanceButton() : null;
                var singleplayerOption = new EditorChoice<>(
                        i18n("settings.game.quick_play.singleplayer"),
                        QuickPlayType.SINGLEPLAYER,
                        singleplayerTextField,
                        singleplayerInheritanceButton);

                var realmsTextField = new JFXTextField();
                @Nullable JFXButton realmsInheritanceButton = !isPresetSetting ? createInheritanceButton() : null;
                var realmsOption = new EditorChoice<>(
                        i18n("settings.game.quick_play.realms"),
                        QuickPlayType.REALMS,
                        realmsTextField,
                        realmsInheritanceButton);
                ...
                bindInheritableStringValue(multiplayerTextField.textProperty(), GameSettings::quickPlayMultiplayerProperty, multiplayerInheritanceButton);
                bindInheritableStringValue(singleplayerTextField.textProperty(), GameSettings::quickPlaySingleplayerProperty, singleplayerInheritanceButton);
                bindInheritableStringValue(realmsTextField.textProperty(), GameSettings::quickPlayRealmsProperty, realmsInheritanceButton);
    ...
    private void bindWindowSizeComboBox(JFXComboBox<String> comboBox, @Nullable JFXButton inheritButton) {
        ...
        ObjectProperty<@Nullable InheritableProperty<Double>> activeParentWidthProperty = new SimpleObjectProperty<>();
        ObjectProperty<@Nullable InheritableProperty<Double>> activeParentHeightProperty = new SimpleObjectProperty<>();
        ...
        InvalidationListener propertyListener = observable -> {
            S setting = currentSetting.get();
            updateParentInheritablePropertyListener(setting, activeParentWidthProperty, GameSettings::widthProperty, refreshHolder.value);
            updateParentInheritablePropertyListener(setting, activeParentHeightProperty, GameSettings::heightProperty, refreshHolder.value);

            @Nullable InheritableProperty<Double> widthProperty = activeWidthProperty.get();
            @Nullable InheritableProperty<Double> heightProperty = activeHeightProperty.get();
            if (widthProperty == null || heightProperty == null || updating.value) {
                if (inheritButton != null && !updating.value) {
                    updateInheritanceButton(inheritButton, true);
                }
                return;
            }
            ...
            try {
                ...
                if (inheritButton != null) {
                    updateInheritanceButton(inheritButton, setting == null || !isWindowSizeOverridden(setting));
                }
            } finally {
        ...
        if (inheritButton != null) {
            inheritButton.addEventFilter(MouseEvent.MOUSE_CLICKED, event -> {
                ...
                updating.value = true;
                try {
                    setWindowSizeOverridden(setting, !isWindowSizeOverridden(setting));
                } finally {
                    updating.value = false;
                }
        ...
        currentSetting.addListener((observable, oldValue, newValue) -> {
            ...
            if (oldWidthProperty != null) {
                oldWidthProperty.removeListener(weakPropertyListener);
            }
            if (oldHeightProperty != null) {
                oldHeightProperty.removeListener(weakPropertyListener);
            }
            ...
            if (newWidthProperty != null) {
                newWidthProperty.addListener(weakPropertyListener);
            }
            if (newHeightProperty != null) {
                newHeightProperty.addListener(weakPropertyListener);
            }
        ...
        S setting = currentSetting.get();
        if (setting != null) {
            InheritableProperty<Double> widthProperty = setting.widthProperty();
            InheritableProperty<Double> heightProperty = setting.heightProperty();
    ...
    private void applyWindowSizeComboBoxValue(JFXComboBox<String> comboBox,
                                              ...
                                              @Nullable InheritableProperty<Double> widthProperty,
                                              @Nullable InheritableProperty<Double> heightProperty,
    ...
    private void setWindowSizeOverridden(@Nullable GameSettings setting) {
        ...
        setWindowSizeOverridden(setting, true);
    }

    /// Returns whether the windowed resolution uses direct width or height values.
    private static boolean isWindowSizeOverridden(GameSettings setting) {
        return isPropertyOverridden(setting, setting.widthProperty())
                || isPropertyOverridden(setting, setting.heightProperty());
    }

    /// Updates whether the windowed resolution uses direct width and height values.
    private void setWindowSizeOverridden(GameSettings setting, boolean overridden) {
    ...
    private <T> void bindSettingBidirectional(Property<T> property, Function<S, Property<T>> propertyGetter) {
        final Holder<Boolean> updating = new Holder<>(false);
        ChangeListener<T> settingListener = (observable, oldValue, newValue) -> {
            if (updating.value) {
                return;
            }
        ...
        ChangeListener<T> weakSettingListener = holder.weak(settingListener);
        ...
        property.addListener((observable, oldValue, newValue) -> {
            ...
            updating.value = true;
            try {
                propertyGetter.apply(setting).setValue(newValue);
            } finally {
                updating.value = false;
            }
        });
        ...
        currentSetting.addListener((observable, oldValue, newValue) -> {
            if (oldValue != null) {
                propertyGetter.apply(oldValue).removeListener(weakSettingListener);
            }

            if (newValue != null) {
                Property<T> settingProperty = propertyGetter.apply(newValue);
                settingProperty.addListener(weakSettingListener);
                updating.value = true;
                try {
                    property.setValue(settingProperty.getValue());
                } finally {
                    updating.value = false;
                }
            }
        ...
        S setting = currentSetting.get();
        if (setting != null) {
            Property<T> settingProperty = propertyGetter.apply(setting);
            settingProperty.addListener(weakSettingListener);
            updating.value = true;
            try {
                property.setValue(settingProperty.getValue());
    ...
    /// Binds a string value to an inheritable setting with an optional inheritance button.
    private void bindInheritableStringValue(
        ...
        InvalidationListener refresh = observable -> {
            ...
            try {
                textProperty.setValue(getEffectiveValue(setting, propertyGetter));
                if (inheritButton != null) {
                    updateInheritanceButton(inheritButton, !isPropertyOverridden(setting, property));
                }
            } finally {
                updating.value = false;
            }
        ...
        textProperty.addListener((observable, oldValue, newValue) -> {
            ...
            try {
                ...
                property.setValue(newValue != null ? newValue : "");
                if (inheritButton != null) {
                    updateInheritanceButton(inheritButton, false);
                }
            } finally {
                updating.value = false;
            }
    ...
    private void bindRunningDirectoryProperty(
        ...
        currentSetting.addListener((observable, oldValue, newValue) -> {
            ...
            if (newValue instanceof GameSettings.Instance newInstance) {
                newInstance.addListener(weakRefresh);
            }
            refresh.invalidated(newValue);
        });
    ...
    private <T> void bindInheritableRadioChoiceList(
        ...
        currentSetting.addListener((observable, oldValue, newValue) -> {
            ...
            InheritableProperty<T> oldProperty = activeProperty.get();
            if (oldProperty != null) {
                oldProperty.removeListener(weakPropertyListener);
            }
            ...
            if (newProperty != null) {
                newProperty.addListener(weakPropertyListener);
            }
        ...
        if (setting != null) {
            ...
            activeProperty.set(property);
            setting.addListener(weakPropertyListener);
            property.addListener(weakPropertyListener);
            propertyListener.invalidated(property);
        }
    }
    ...
    private void bindWindowSettings(ComponentSublist sublist, RadioChoiceList<GameWindowType> item) {
        ...
        currentSetting.addListener((observable, oldValue, newValue) -> {
            ...
            if (oldWindowTypeProperty != null) {
                oldWindowTypeProperty.removeListener(weakPropertyListener);
            }
            ...
            if (newWindowTypeProperty != null) {
                newWindowTypeProperty.addListener(weakPropertyListener);
            }
        ...
        if (setting != null) {
            ...
            setting.addListener(weakPropertyListener);
            setting.windowTypeProperty().addListener(weakPropertyListener);
        }
        propertyListener.invalidated(setting);
    }

    /// Returns whether the window type uses a direct value.
    ...
    /// Updates whether the window type uses a direct property value.
    ...
    private <T> void bindInheritableSublistDescription(ComponentSublist sublist,
        ...
        currentSetting.addListener((observable, oldValue, newValue) -> {
            if (oldValue != null) {
                propertyGetter.apply(oldValue).removeListener(weakPropertyListener);
            }
    ...
    private <T> void bindInheritableLineSelectButton(
        ...
        if (setting != null) {
            ...
            setting.addListener(weakRefresh);
            activeProperty.get().addListener(weakRefresh);
            refresh.invalidated(setting);
        }
    }
    ...
    @SuppressWarnings("unchecked")
    @Override
    public void loadInstance(SCLGameRepository repository, @Nullable String instanceId) {
    ...
    private void refreshJavaSettings() {
        S setting = currentSetting.get();
        updateParentInheritablePropertyListener(setting, activeParentJavaTypeProperty, GameSettings::javaTypeProperty, weakJavaListener);
        updateParentInheritablePropertyListener(setting, activeParentCustomJavaVersionProperty, GameSettings::customJavaVersionProperty, weakJavaListener);
        updateParentInheritablePropertyListener(setting, activeParentCustomJavaPathProperty, GameSettings::customJavaPathProperty, weakJavaListener);
        updateParentInheritablePropertyListener(setting, activeParentDetectedJavaProperty, GameSettings::detectedJavaProperty, weakJavaListener);
        initJavaSubtitle();
    }
