...
import studio.lively.scl.download.game.GameAssetIndexDownloadTask;
import studio.lively.scl.download.game.LibraryDownloadException;
...
import studio.lively.scl.task.DownloadException;
...
public final class UpdateInstallerWizardProvider implements WizardProvider {
    ...
    public UpdateInstallerWizardProvider(@NotNull SCLGameRepository repository, @NotNull String gameVersion, @NotNull Version version, @NotNull String libraryId, @Nullable String oldLibraryVersion) {
    ...
    public Object finish(SettingsMap settings) {
        ...
        for (Object value : settings.asStringMap().values()) {
            if (value instanceof RemoteVersion remoteVersion) {
                ...
                hints.add(new Task.StagesHint(String.format("scl.install.%s:%s", remoteVersion.getLibraryId(), remoteVersion.getSelfVersion())));
                if ("game".equals(remoteVersion.getLibraryId())) {
                    hints.add(new Task.StagesHint("scl.install.libraries"));
                    hints.add(new Task.StagesHint("scl.install.assets"));
                }
