...
import studio.lively.scl.game.SCLCacheRepository;
import studio.lively.scl.setting.*;
import studio.lively.scl.task.AsyncTaskExecutor;
...
import studio.lively.scl.upgrade.UpdateHandler;
...
public final class Launcher extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        try {
            ...
            if (Metadata.SCL_LOCAL_HOME.toString().indexOf('=') >= 0) {
    ...
    private static void initializeSettingsRuntime() {
        ...
        CacheRepository.setInstance(SCLCacheRepository.REPOSITORY);
        SCLCacheRepository.REPOSITORY.directoryProperty().bind(Bindings.createStringBinding(() -> {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            LOG.info("SCL User Home: " + Metadata.SCL_USER_HOME);
            LOG.info("SCL Local Home: " + Metadata.SCL_LOCAL_HOME);
            LOG.info("SCL Jar Path: " + Lang.requireNonNullElse(JarUtils.thisJarPath(), "Not Found"));
            LOG.info("SCL Log File: " + Lang.requireNonNullElse(LOG.getLogFile(), "In Memory"));
    ...
    private static void setupJavaFXVMOptions() {
        if ("true".equalsIgnoreCase(System.getenv("SCL_FORCE_GPU"))) {
            LOG.info("SCL_FORCE_GPU: true");
            System.getProperties().putIfAbsent("prism.forceGPU", "true");
        }

        setUpAnimationFrameRate:
        {
            String animationFrameRate = System.getenv("SCL_ANIMATION_FRAME_RATE");
            if (animationFrameRate != null) {
                LOG.info("SCL_ANIMATION_FRAME_RATE: " + animationFrameRate);
        ...
        String uiScale = System.getProperty("scl.uiScale", System.getenv("SCL_UI_SCALE"));
        if (uiScale != null) {
            uiScale = uiScale.trim();

            LOG.info("SCL_UI_SCALE: " + uiScale);
    ...
    private static void checkConfigOwner() {
        ...
        if (Files.exists(Metadata.SCL_USER_HOME))
            files.add(Metadata.SCL_USER_HOME.toString());
