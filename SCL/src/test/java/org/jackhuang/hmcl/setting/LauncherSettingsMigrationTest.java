...
public final class LauncherSettingsMigrationTest {
    ...
    /// Tests migrating the SCL 2.x selected offline account username into the current account ID.
    @Test
    public void migratesLegacySCL2SelectedOfflineAccount() throws IOException {
        try (FileSystem fileSystem = Jimfs.newFileSystem(Configuration.unix())) {
            Path root = fileSystem.getPath("/launcher-settings-migration-tests");
            Files.createDirectories(root);
    ...
    public void preservesPatchSchemaAndUnknownFields() {
        LauncherSettings launcherSettings = Objects.requireNonNull(LauncherSettings.fromJson(JsonParser.parseString("""
                {
                  "$schema": "https://raw.githubusercontent.com/lively-Studio/Slime-Craft-Launcher/main/docs/schemas/launcher-settings/1.0.1",
                  "futureField": true
                }
                """).getAsJsonObject()));
        ...
        assertEquals("https://raw.githubusercontent.com/lively-Studio/Slime-Craft-Launcher/main/docs/schemas/launcher-settings/1.0.1",
